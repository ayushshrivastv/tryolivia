...
public final class SecureLogger {
    ...
    private static let cacheQueue = DispatchQueue(label: "chat.olivia.securelogger.cache", attributes: .concurrent)
    ...
    /// Minimum level that will be logged. Defaults to .info. Override via env OLIVIA_LOG_LEVEL.
    private static let minimumLevel: LogLevel = {
        let env = ProcessInfo.processInfo.environment["OLIVIA_LOG_LEVEL"]?.lowercased()
        switch env {
        case "debug": return .debug
