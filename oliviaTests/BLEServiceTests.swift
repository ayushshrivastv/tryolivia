...
final class BLEServiceTests: XCTestCase {
    ...
    func testSendPublicMessage() {
        ...
        let delegate = MockOliviaDelegate { message in
    ...
    func testSimulateIncomingMessage() {
        ...
        let incomingMessage = OliviaMessage(
            id: "MSG456",
            sender: "RemoteUser",
    ...
    func testMessageDeliveryHandler() {
        ...
        service.packetDeliveryHandler = { packet in
            if let msg = OliviaMessage(packet.payload) {
...
private final class MockOliviaDelegate: OliviaDelegate {
    private let messageHandler: (OliviaMessage) -> Void
    
    init(_ handler: @escaping (OliviaMessage) -> Void) {
