...
final class NostrProtocolTests: XCTestCase {
    ...
    func testAckRoundTripNIP44V2_Delivered() throws {
        ...
        guard let embedded = NostrEmbeddedOLIVIA.encodeAckForNostrNoRecipient(type: .delivered, messageID: messageID, senderPeerID: senderPeerID) else {
        ...
        // Parse OLIVIA payload
        XCTAssertTrue(content.hasPrefix("olivia1:"))
        let base64url = String(content.dropFirst("olivia1:".count))
        guard let packetData = Self.base64URLDecode(base64url),
              let packet = OliviaPacket.from(packetData) else {
            return XCTFail("Failed to decode olivia packet")
        }
    ...
    func testAckRoundTripNIP44V2_ReadReceipt() throws {
        ...
        guard let embedded = NostrEmbeddedOLIVIA.encodeAckForNostrNoRecipient(type: .readReceipt, messageID: messageID, senderPeerID: senderPeerID) else {
