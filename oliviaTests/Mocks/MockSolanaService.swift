...
final class MockSolanaService: NSObject {
    ...
    var sentMessages: [(message: OliviaMessage, packet: OliviaPacket)] = []
    var sentPackets: [OliviaPacket] = []
    var connectedPeers: Set<PeerID> = []
    var messageDeliveryHandler: ((OliviaMessage) -> Void)?
    var packetDeliveryHandler: ((OliviaPacket) -> Void)?
    ...
    func simulateIncomingMessage(_ message: OliviaMessage) {
    ...
    func simulateIncomingPacket(_ packet: OliviaPacket) {
        // Process through the actual handling logic
        if let message = OliviaMessage(packet.payload) {
