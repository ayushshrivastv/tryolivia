...
/// SwiftUI views and the underlying OLIVIA services. It manages:
...
/// Manages the application state and business logic for OLIVIA.
/// Acts as the primary coordinator between UI components and backend services,
/// implementing the OliviaDelegate protocol to handle network events.
final class ChatViewModel: ObservableObject, OliviaDelegate {
    ...
    @MainActor
    private func normalizedSenderKey(for message: OliviaMessage) -> String {
    ...
    @Published var messages: [OliviaMessage] = []
    ...
    @Published var allPeers: [OliviaPeer] = []
    var privateChats: [String: [OliviaMessage]] { 
    ...
    private var peerIndex: [String: OliviaPeer] = [:]
    ...
    private var geoTimelines: [String: [OliviaMessage]] = [:] // geohash -> messages
    ...
    private var publicBuffer: [OliviaMessage] = []
    ...
    init(
        ...
        let peersCancellable = unifiedPeerService.$peers
            ...
            .sink { [weak self] peers in
                ...
                var uniquePeers: [String: OliviaPeer] = [:]
                for peer in peers {
    ...
    private func subscribeGiftWrap(_ giftWrap: NostrEvent, id: NostrIdentity) {
        ...
        guard let (content, senderPubkey, rumorTs) = try? NostrProtocol.decryptPrivateMessage(giftWrap: giftWrap, recipientIdentity: id),
              content.hasPrefix("olivia1:"),
              let packetData = Self.base64URLDecode(String(content.dropFirst("olivia1:".count))),
              let packet = OliviaPacket.from(packetData),
    ...
    /// Sends a message through the OLIVIA network.
    ...
    private func handleGiftWrap(_ giftWrap: NostrEvent, id: NostrIdentity) {
        ...
        guard content.hasPrefix("olivia1:"),
              let packetData = Self.base64URLDecode(String(content.dropFirst("olivia1:".count))),
              let packet = OliviaPacket.from(packetData),
              packet.type == MessageType.noiseEncrypted.rawValue,
                ...
                if let nostrMessages = privateChats[noiseKeyHex], !nostrMessages.isEmpty {
                    ...
                    for message in nostrMessages {
                        if !existingMessageIds.contains(message.id) {
                            ...
                            let updatedMessage = OliviaMessage(
                                id: message.id,
    ...
    @objc private func handleNostrMessage(_ notification: Notification) {
        guard let message = notification.userInfo?["message"] as? OliviaMessage else { return }
    ...
    @objc private func userDidTakeScreenshot() {
        ...
        if let peerID = selectedPrivateChatPeer {
            ...
            let localNotification = OliviaMessage(
                sender: "system",
    ...
    @MainActor
    func getPrivateChatMessages(for peerID: String) -> [OliviaMessage] {
        var combined: [OliviaMessage] = []
        ...
        var bestByID: [String: OliviaMessage] = [:]
        for msg in combined {
    ...
    func panicClearAllData() {
        ...
        userDefaults.removeObject(forKey: "olivia.noiseIdentityKey")
        userDefaults.removeObject(forKey: "olivia.messageRetentionKey")
    ...
    func formatMessageAsText(_ message: OliviaMessage, colorScheme: ColorScheme) -> AttributedString {
        ...
        if message.sender != "system" {
            ...
            senderStyle.font = .oliviaSystem(size: 14, weight: fontWeight, design: .monospaced)
            // Make sender clickable: encode senderPeerID into a custom URL
            if let spid = message.senderPeerID?.id, let url = URL(string: "olivia://user/\(spid.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? spid)") {
            ...
            if (content.count > 4000 || content.hasVeryLongToken(threshold: 1024)) && !containsCashuEarly {
                ...
                plainStyle.font = isSelf
                    ? .oliviaSystem(size: 14, weight: .bold, design: .monospaced)
                    : .oliviaSystem(size: 14, design: .monospaced)
            ...
            for (range, type) in allMatches {
                ...
                if let nsRange = Range(range, in: content) {
                    ...
                    if type == "mention" {
                        ...
                        mentionStyle.font = .oliviaSystem(size: 14, weight: isSelf ? .bold : .semibold, design: .monospaced)
                        ...
                        if type == "hashtag" {
                            // If the hashtag is a valid geohash, make it tappable (olivia://geohash/<gh>)
                            ...
                            if isGeohash && !attachedToMention && standalone, let url = URL(string: "olivia://geohash/\(token)") {
                                tagStyle.link = url
                            ...
                            matchStyle.font = .oliviaSystem(size: 14, weight: isSelf ? .bold : .semibold, design: .monospaced)
                            if type == "url" {
            ...
            timestampStyle.font = .oliviaSystem(size: 10, design: .monospaced)
            ...
            contentStyle.font = .oliviaSystem(size: 12, design: .monospaced).italic()
    ...
    func formatMessage(_ message: OliviaMessage, colorScheme: ColorScheme) -> AttributedString {
        var result = AttributedString()
        ...
        if message.sender == "system" {
            ...
            senderStyle.font = .oliviaSystem(size: 12, weight: fontWeight, design: .monospaced)
                ...
                if let range = Range(match.range(at: 0), in: contentText) {
                    if lastEndIndex < range.lowerBound {
                        ...
                        if !beforeText.isEmpty {
                            ...
                            normalStyle.font = .oliviaSystem(size: 14, design: .monospaced)
                    ...
                    var mentionStyle = AttributeContainer()
                    mentionStyle.font = .oliviaSystem(size: 14, weight: .semibold, design: .monospaced)
            ...
            if message.isRelay, let originalSender = message.originalSender {
                ...
                relayStyle.font = .oliviaSystem(size: 11, design: .monospaced)
    ...
    @MainActor
    private func peerColor(for message: OliviaMessage, isDark: Bool) -> Color {
        if let spid = message.senderPeerID?.id {
            if spid.hasPrefix("nostr:") || spid.hasPrefix("nostr_") {
                let bare: String = {
                    if spid.hasPrefix("nostr:") { return String(spid.dropFirst(6)) }
    ...
    private func addMessage(_ message: OliviaMessage) {
    ...
    func getPeer(byID peerID: String) -> OliviaPeer? {
    ...
    func didReceiveMessage(_ message: OliviaMessage) {
        Task { @MainActor in
            // Early validation
    ...
    func didUpdatePeerList(_ peers: [PeerID]) {
        ...
        DispatchQueue.main.async {
            ...
            // Smart notification logic for "oliviaters nearby"
            if !peers.isEmpty {
    ...
    private func updateMessageDeliveryStatus(_ messageID: String, status: DeliveryStatus) {
        ...
        for (peerID, chatMessages) in privateChats {
            ...
            // Update delivery status directly (OliviaMessage is a class/reference type)
    ...
    private func processNostrMessage(_ giftWrap: NostrEvent) {
        ...
        do {
            ...
            // Expect embedded OLIVIA packet content
            guard content.hasPrefix("olivia1:") else {
                SecureLogger.debug("Ignoring non-embedded Nostr DM content", category: .session)
                return
            }

            guard let packetData = Self.base64URLDecode(String(content.dropFirst("olivia1:".count))),
                  let packet = OliviaPacket.from(packetData) else {
                SecureLogger.error("Failed to decode embedded OLIVIA packet from Nostr DM", category: .session)
                return
            }
    ...
    private func addMessageToPrivateChatsIfNeeded(_ message: OliviaMessage, targetPeerID: String) {
    ...
    @MainActor
    private func mirrorToEphemeralIfNeeded(_ message: OliviaMessage, targetPeerID: String, key: Data?) {
        guard let key,
    ...
    @MainActor
    private func sendDeliveryAckViaNostrEmbedded(_ message: OliviaMessage, wasReadBefore: Bool, senderPubkey: String, key: Data?) {
    ...
    @MainActor
    private func handleViewingThisChat(_ message: OliviaMessage, targetPeerID: String, key: Data?, senderPubkey: String) {
    ...
    private func processActionMessage(_ message: OliviaMessage) -> OliviaMessage {
    ...
    private func migratePrivateChatsIfNeeded(for peerID: String, senderNickname: String) {
        ...
        if privateChats[peerID] == nil || privateChats[peerID]?.isEmpty == true {
            var migratedMessages: [OliviaMessage] = []
    ...
    @MainActor
    private func handlePrivateMessage(_ message: OliviaMessage) {
    ...
    @MainActor
    private func handlePublicMessage(_ message: OliviaMessage) {
    ...
    @MainActor
    private func enqueuePublic(_ message: OliviaMessage) {
...
private func checkForMentions(_ message: OliviaMessage) {
...
/// Send haptic feedback for special messages (iOS only)
    private func sendHapticFeedback(for message: OliviaMessage) {
        #if os(iOS)
        guard UIApplication.shared.applicationState == .active else { return }
