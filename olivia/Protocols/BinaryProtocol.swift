...
/// Implements binary encoding and decoding for OLIVIA protocol messages.
/// Provides static methods for converting between OliviaPacket objects and
...
struct BinaryProtocol {
    ...
    // Encode OliviaPacket to binary format
    static func encode(_ packet: OliviaPacket, padding: Bool = true) -> Data? {
        var data = Data()
    ...
    // Decode binary data to OliviaPacket
    static func decode(_ data: Data) -> OliviaPacket? {
    ...
    private static func decodeCore(_ raw: Data) -> OliviaPacket? {
        // Minimum size: header + senderID
        guard raw.count >= headerSize + senderIDSize else { return nil }

        return raw.withUnsafeBytes { (buf: UnsafeRawBufferPointer) -> OliviaPacket? in
