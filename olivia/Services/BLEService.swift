...
extension BLEService {
    func _test_handlePacket(_ packet: OliviaPacket, fromPeerID: PeerID) {
...
extension BLEService: CBPeripheralDelegate {
    ...
    private func processNotificationPacket(_ packet: OliviaPacket, from peripheral: CBPeripheral, peripheralUUID: String) {
    ...
    func peripheral(_ peripheral: CBPeripheral, didModifyServices invalidatedServices: [CBService]) {
        ...
        if !hasOurService {
            ...
            SecureLogger.warning("❌ OLIVIA service removed - disconnecting from \(peripheral.name ?? peripheral.identifier.uuidString)", category: .session)
            centralManager?.cancelPeripheralConnection(peripheral)
        } else {
            // Try to rediscover
            peripheral.discoverServices([BLEService.serviceUUID])
...
extension BLEService {
    ...
    private func makeMessageID(for packet: OliviaPacket) -> String {
    ...
    private func sendEncrypted(_ packet: OliviaPacket, data: Data, pad: Bool) {
    ...
    private func sendGenericBroadcast(_ packet: OliviaPacket, data: Data, pad: Bool) {
        sendOnAllLinks(packet: packet, data: data, pad: pad, directedOnlyPeer: nil)
    }

    private func sendOnAllLinks(packet: OliviaPacket, data: Data, pad: Bool, directedOnlyPeer: PeerID?) {
    ...
    private func sendPacketDirected(_ packet: OliviaPacket, to peerID: PeerID) {
    ...
    private func spoolDirectedPacket(_ packet: OliviaPacket, recipientPeerID: PeerID) {
        let msgID = makeMessageID(for: packet)
        collectionsQueue.async(flags: .barrier) { [weak self] in
            guard let self = self else { return }
    ...
    private func flushDirectedSpool() {
        ...
        let toSend: [(PeerID, OliviaPacket)] = collectionsQueue.sync(flags: .barrier) {
            var out: [(PeerID, OliviaPacket)] = []
            let now = Date()
    ...
    private func rebroadcastRecentAnnounces() {
        ...
        let packets: [OliviaPacket] = collectionsQueue.sync {
    ...
    private func sendFragmentedPacket(_ packet: OliviaPacket, pad: Bool, maxChunk: Int? = nil, directedOnlyPeer: PeerID? = nil) {
        ...
        for (index, fragment) in fragments.enumerated() {
            ...
            let fragmentPacket = OliviaPacket(
    ...
    private func handleFragment(_ packet: OliviaPacket, from peerID: PeerID) {
    ...
    private func handleReceivedPacket(_ packet: OliviaPacket, from peerID: PeerID) {
    ...
    private func handleAnnounce(_ packet: OliviaPacket, from peerID: PeerID) {
    ...
    private func handleRequestSync(_ packet: OliviaPacket, from peerID: PeerID) {
    ...
    private func handleMessage(_ packet: OliviaPacket, from peerID: PeerID) {
    ...
    private func handleNoiseHandshake(_ packet: OliviaPacket, from peerID: PeerID) {
           ...
           recipientID.hexEncodedString() == myPeerID {
            ...
            do {
                if let response = try noiseService.processHandshakeMessage(from: peerID, message: packet.payload) {
                    // Send response
                    let responsePacket = OliviaPacket(
                        type: MessageType.noiseHandshake.rawValue,
                        senderID: myPeerIDData,
                        recipientID: Data(hexString: peerID.id),
                        timestamp: UInt64(Date().timeIntervalSince1970 * 1000),
    ...
    private func handleNoiseEncrypted(_ packet: OliviaPacket, from peerID: PeerID) {
    ...
    private func handleLeave(_ packet: OliviaPacket, from peerID: PeerID) {
    ...
    private func performCleanup() {
        ...
        collectionsQueue.async(flags: .barrier) { [weak self] in
            ...
            if !self.pendingDirectedRelays.isEmpty {
                var cleaned: [PeerID: [String: (packet: OliviaPacket, enqueuedAt: Date)]] = [:]
