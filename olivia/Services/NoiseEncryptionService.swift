...
/// Manages end-to-end encryption for OLIVIA using the Noise Protocol Framework.
...
final class NoiseEncryptionService {
    ...
    // Thread safety
    private let serviceQueue = DispatchQueue(label: "chat.olivia.noise.service", attributes: .concurrent)
    ...
    private func canonicalAnnounceBytes(peerID: Data, noiseKey: Data, ed25519Key: Data, nickname: String, timestampMs: UInt64) -> Data {
        var out = Data()
        // context
        let context = "olivia-announce-v1".data(using: .utf8) ?? Data()
    ...
    /// Sign a OliviaPacket using the noise private key
    func signPacket(_ packet: OliviaPacket) -> OliviaPacket? {
    ...
    /// Verify a OliviaPacket signature using the provided public key
    func verifyPacketSignature(_ packet: OliviaPacket, publicKey: Data) -> Bool {
...
/// These types wrap the underlying OLIVIA protocol messages with encryption metadata.
