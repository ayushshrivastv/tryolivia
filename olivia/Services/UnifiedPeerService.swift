...
final class UnifiedPeerService: ObservableObject, TransportPeerEventsDelegate {
    ...
    @Published private(set) var peers: [OliviaPeer] = []
    @Published private(set) var connectedPeerIDs: Set<PeerID> = []
    @Published private(set) var favorites: [OliviaPeer] = []
    @Published private(set) var mutualFavorites: [OliviaPeer] = []
    
    // MARK: - Private Properties
    
    private var peerIndex: [PeerID: OliviaPeer] = [:]
    ...
    private func updatePeers() {
        ...
        enrichedPeers.sort { lhs, rhs in
            ...
            func rank(_ p: OliviaPeer) -> Int { p.isConnected ? 2 : (p.isReachable ? 1 : 0) }
        ...
        var favoritesList: [OliviaPeer] = []
        var mutualsList: [OliviaPeer] = []
        var newIndex: [PeerID: OliviaPeer] = [:]
    ...
    /// Get peer by ID
    func getPeer(by peerID: PeerID) -> OliviaPeer? {
