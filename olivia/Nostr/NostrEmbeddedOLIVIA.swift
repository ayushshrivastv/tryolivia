...
// MARK: - OLIVIA-over-Nostr Adapter

struct NostrEmbeddedOLIVIA {
    /// Build a `olivia1:` base64url-encoded OLIVIA packet carrying a private message for Nostr DMs.
    static func encodePMForNostr(content: String, messageID: String, recipientPeerID: String, senderPeerID: String) -> String? {
        ...
        return "olivia1:" + base64URLEncode(data)
    }

    /// Build a `olivia1:` base64url-encoded OLIVIA packet carrying a delivery/read ack for Nostr DMs.
    ...
    /// Build a `olivia1:` ACK (delivered/read) without an embedded recipient peer ID (geohash DMs).
    ...
    /// Build a `olivia1:` payload without an embedded recipient peer ID (used for geohash DMs).
