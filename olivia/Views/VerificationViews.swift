    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(Strings.pastePrompt)
                .font(.oliviaSystem(size: 14, weight: .medium, design: .monospaced))
                ...
                if showingScanner {
                    Button(action: { showingScanner = false }) {
                        Label("show my qr", systemImage: "qrcode")
                            .font(.oliviaSystem(size: 13, design: .monospaced))
                    }
                    .buttonStyle(.bordered)
                } else {
                    Button(action: { showingScanner = true }) {
                        Label("scan someone else's qr", systemImage: "camera.viewfinder")
                            .font(.oliviaSystem(size: 13, weight: .medium, design: .monospaced))
                    }
                    .buttonStyle(.bordered)
                    .tint(.gray)
                }
