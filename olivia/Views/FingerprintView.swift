    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            HStack {
                ...
                Button(action: { dismiss() }) {
                    Image(systemName: "xmark")
                        .font(.oliviaSystem(size: 14, weight: .semibold))
                }
                .foregroundColor(textColor)
            }
            .padding()
            ...
            VStack(alignment: .leading, spacing: 16) {
                // Prefer short network ID for session/encryption status
                ...
                HStack {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text(peerNickname)
                            .font(.oliviaSystem(size: 18, weight: .semibold, design: .monospaced))
                            .foregroundColor(textColor)
                        
                        Text(encryptionStatus.description)
                            .font(.oliviaSystem(size: 12, design: .monospaced))
                ...
                VStack(alignment: .leading, spacing: 8) {
                    Text(Strings.theirFingerprint)
                        .font(.oliviaSystem(size: 12, weight: .bold, design: .monospaced))
                        .foregroundColor(textColor.opacity(0.7))
                ...
                if encryptionStatus == .noiseSecured || encryptionStatus == .noiseVerified {
                    ...
                    VStack(spacing: 12) {
                        Text(isVerified ? Strings.verifiedBadge : Strings.notVerifiedBadge)
                            .font(.oliviaSystem(size: 14, weight: .bold, design: .monospaced))
