...
struct ContentView: View {
    ...
    private func messagesView(privatePeer: String?, isAtBottom: Binding<Bool>) -> some View {
        ScrollViewReader { proxy in
            ...
            .background(backgroundColor)
            .onOpenURL { url in
                guard url.scheme == "olivia", url.host == "user" else { return }
            ...
            .onOpenURL { url in
                guard url.scheme == "olivia", url.host == "geohash" else { return }
    ...
    private var inputView: some View {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ForEach(Array(viewModel.autocompleteSuggestions.prefix(4)), id: \.self) { suggestion in
                        Button(action: {
                            ...
                            HStack {
                                Text(suggestion)
                                    .font(.oliviaSystem(size: 11, design: .monospaced))
                                    .foregroundColor(textColor)
                                    .fontWeight(.medium)
                                Spacer()
                            }
                            ...
                            Button(action: {
                                ...
                                HStack {
                                    ...
                                    if let syntax = info.syntax {
                                        Text(syntax)
                                            .font(.oliviaSystem(size: 10, design: .monospaced))
        ...
        TextField("content.input.message_placeholder", text: $messageText)
                ...
                .font(.oliviaSystem(size: 14, design: .monospaced))
                .foregroundColor(textColor)
    ...
    private var mainHeaderView: some View {
        HStack(spacing: 0) {
            Text(verbatim: "olivia/")
                .font(.oliviaSystem(size: 18, weight: .medium, design: .monospaced))
                .foregroundColor(textColor)
                .onTapGesture(count: 3) {
                        ...
                        HStack {
                            ...
                            Button(action: { showLocationNotes = false }) {
                                Image(systemName: "xmark")
                                    .font(.oliviaSystem(size: 13, weight: .semibold, design: .monospaced))
                                    .foregroundColor(textColor)
                    ...
                    Button(action: {
                        ...
                        HStack(spacing: 6) {
                            ...
                            Text("\(privatePeerNick)")
                                .font(.oliviaSystem(size: 16, weight: .medium, design: .monospaced))
