...
- TorManager
  ...
  - Fails closed by default until Tor is bootstrapped. For local development only, define OLIVIA_DEV_ALLOW_CLEARNET to bypass Tor.
- TorURLSession
...
torrc template
The generated torrc (under Application Support/olivia/tor/torrc) is:

  DataDirectory <AppSupport>/olivia/tor
  ClientOnly 1
...
- To temporarily allow direct network without Tor for local development:
  - Add Swift compiler flag: OLIVIA_DEV_ALLOW_CLEARNET
